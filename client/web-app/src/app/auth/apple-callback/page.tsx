  ...
  useEffect(() => {
    const processAppleCallback = async () => {
      try {
        ...
        if (!code && !idToken) {
          throw new Error('No authorization code or ID token received from Apple');
        }
        ...
        if (idToken) {
          ...
          if (result.data?.signInWithApple.success && result.data.signInWithApple.sessionToken) {
            ...
            throw new Error(result.data?.signInWithApple.error || 'Sign in failed');
          }
        } else {
      ...
      <Card className="p-8 max-w-md w-full text-center">
        {status === 'loading' && (
          <>
            ...
            <h2 className="text-xl font-semibold mb-2">Processing Apple Sign In</h2>
