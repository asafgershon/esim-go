...
import { DollarSign, TrendingUp, Clock, Percent, ChevronRight, ChevronDown, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
const transformDataForTable = (countries: CountryGroupData[], expandedCountries: Set<string>) => {
  ...
  countries.forEach(country => {
    ...
    if (expandedCountries.has(country.countryId) && country.bundles && country.bundles.length > 0) {
      // Sort bundles by duration before adding them
      const sortedBundles = [...country.bundles].sort((a, b) => a.duration - b.duration);
...
export function CountryPricingTableGrouped({
  ...
  const tableData = useMemo(() => transformDataForTable(countries, expandedCountries), [countries, expandedCountries]);
  ...
  const columns: ColumnDef<PricingData>[] = useMemo(() => enableColumnGrouping([
    {
      id: 'expand',
      header: '',
      cell: ({ row }) => {
        ...
        const country = countries.find(c => c.countryId === data.countryId);
    ...
    },
    {
      ...
      sortingFn: (rowA, rowB) => {
        ...
        if (a.duration === 0 && b.duration === 0) {
          const countryA = countries.find(c => c.countryId === a.countryId);
          const countryB = countries.find(c => c.countryId === b.countryId);
          return (countryA?.totalBundles || 0) - (countryB?.totalBundles || 0);
        }
        ...
        return (
          <div className="space-y-1">
            <div className="flex items-center gap-1">
              ...
              <span className="font-medium">{formatCurrency(data.pricePerDay)}</span>
            </div>
        ...
        return (
          <div className="space-y-1">
            <div className="flex items-center gap-1">
              ...
              <Badge variant={data.hasCustomDiscount ? "default" : "outline"}>
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="space-y-1 text-sm">
            ...
            <div className="font-medium">Total: {formatCurrency(data.totalCost)}</div>
          </div>
        );
      },
    },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const data = row.original;
        const profitMargin = ((data.finalRevenue - data.totalCost) / data.totalCost) * 100;
        return (
          <div className="space-y-1">
  ...
  ], ['countryName']), [countries, onCountryClick, onBundleClick, expandedCountries, loadingCountries]);
    ...
    createFilteringPlugin({
      ...
      quickFilters: [
        ...
        },
        {
          ...
          value: 'high-margin',
          filter: (row) => ((row.finalRevenue - row.totalCost) / row.totalCost) > 0.5,
        },
      ],
    }),
  ], []);
  ...
  const stats = useMemo(() => {
    ...
    const totalRevenue = tableData.reduce((sum, bundle) => sum + bundle.finalRevenue, 0);
    const avgRevenue = totalRevenue / totalBundles;
    const countriesWithBundles = countries.filter(c => c.bundles && c.bundles.length > 0);
