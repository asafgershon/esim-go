...
export const EnhancedActionBuilder: React.FC<EnhancedActionBuilderProps> = ({
  ...
  const getAvailableActionsForCategory = (category: RuleCategory): ActionType[] => {
  ...
  const calculatePreviewEffect = (action: RuleAction): string => {
    ...
    switch (action.type) {
      case ActionType.AddMarkup:
        return `$${testPrice.toFixed(2)} → $${(testPrice + action.value).toFixed(2)}`;
      case ActionType.ApplyDiscountPercentage:
        const discountAmount = testPrice * (action.value / 100);
        return `$${testPrice.toFixed(2)} → $${(testPrice - discountAmount).toFixed(2)} (saves $${discountAmount.toFixed(2)})`;
      case ActionType.SetProcessingRate:
        const processingFee = testPrice * (action.value / 100);
        return `Processing fee: $${processingFee.toFixed(2)} (${action.value}% of $${testPrice.toFixed(2)})`;
      ...
      <CardContent className="space-y-4 pt-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <CategoryIcon className={`h-5 w-5 ${getCategoryColor(ruleCategory)}`} />
            <div>
        ...
        {actions.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm">Click "Add Action" to define what this rule does</p>
          </div>
        ) : (
                ...
                <div key={index} className="border rounded-lg p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        value={action.type}
                        onValueChange={(value) => updateAction(index, { type: value as ActionType, value: 0 })}
                      >
                        <SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      {definition?.examples && (
                        <div className="flex flex-wrap gap-1">
                          <span className="text-xs text-gray-500">Quick values:</span>
                          {definition.examples.map((example) => (
                            <Badge
                              key={example}
                              variant="outline"
                              className="cursor-pointer hover:bg-blue-50 text-xs"
                              onClick={() => updateAction(index, { value: example })}
                            >
                              {example}{definition.unit}
                            </Badge>
                          ))}
                        </div>
                      )}
                  ...
                  {/* Action Preview */}
                  {action.type && action.value && !validation && previewEffect && (
                    <div className="bg-green-50 border border-green-200 rounded p-3">
                      <div className="flex items-center gap-2 mb-1">
                        <ActionIcon className="h-4 w-4 text-green-600" />
                        <span className="text-sm font-medium text-green-800">Effect Preview</span>
                      </div>
                      <p className="text-sm text-green-700">{previewEffect}</p>
        ...
        {actions.length > 1 && (
          <div className="bg-blue-50 border border-blue-200 rounded p-3">
            <h4 className="text-sm font-medium text-blue-800 mb-2">Actions Summary</h4>
              ...
              {actions.map((action, index) => {
                ...
                return (
                  <li key={index} className="flex items-center gap-2">
                    ...
                    {definition?.label}: {action.value}{definition?.unit}
                  </li>
                );
              })}
            </ul>
