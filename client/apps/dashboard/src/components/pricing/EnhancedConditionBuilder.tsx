...
const OPERATORS_BY_TYPE = {
  string: [
    ...
    { value: 'NOT_EQUALS', label: 'does not equal', description: 'Not equal to' },
    { value: 'IN', label: 'is one of', description: 'Matches any in list' },
    { value: 'NOT_IN', label: 'is not one of', description: 'Does not match any in list' },
  ],
  number: [
  ...
  boolean: [
    { value: 'EQUALS', label: 'is', description: 'Boolean match' },
  ],
};

export const EnhancedConditionBuilder: React.FC<EnhancedConditionBuilderProps> = ({
  conditions,
  onChange,
  className = '',
}) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [activeFieldPopover, setActiveFieldPopover] = useState<number | null>(null);
  ...
  const getOperatorsForField = (fieldValue: string) => {
    ...
    return OPERATORS_BY_TYPE[field.type as keyof typeof OPERATORS_BY_TYPE] || OPERATORS_BY_TYPE.string;
  };

  const filteredFields = CONDITION_FIELDS.filter(field =>
  ...
  const renderValueInput = (condition: RuleCondition, index: number) => {
    ...
    if (field.type === 'enum' || (field.isDynamic && fieldData.getFieldOptions(condition.field).length > 0)) {
      const options = field.isDynamic 
        ? fieldData.getFieldOptions(condition.field)
        : (field.examples || []).map(ex => ({ value: String(ex), label: String(ex) }));
      
      const isLoading = field.isDynamic && fieldData.isFieldLoading(condition.field);
      const hasError = field.isDynamic && fieldData.hasFieldError(condition.field);
              ...
              <div className="flex flex-wrap gap-1">
                {options.slice(0, 8).map((option) => (
                  <Badge
                    ...
                    onClick={() => {
                      const currentValues = condition.value ? condition.value.split(',').map(v => v.trim()) : [];
              ...
              <div className="flex flex-wrap gap-1">
                ...
                {examples.slice(0, 3).map((example) => (
                  <Badge
                    ...
                    variant="outline"
                    className="cursor-pointer hover:bg-blue-50 text-xs"
                    onClick={() => updateCondition(index, { value: String(example) })}
                  >
                    {example}
                  </Badge>
                ))}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <Popover 
                        ...
                        onOpenChange={(open) => setActiveFieldPopover(open ? index : null)}
                      >
                  ...
                  {condition.field && condition.operator && condition.value && (
                    <div className="bg-blue-50 border border-blue-200 rounded p-3">
                      <p className="text-sm text-blue-800">
                        <strong>Preview:</strong> {field?.label || condition.field} {selectedOperator?.label || condition.operator} "{condition.value}"
                      </p>
                    </div>
                  )}
                </div>
