...
CREATE TABLE IF NOT EXISTS "public"."pricing_rules" (
    ...
    CONSTRAINT "pricing_rules_category_check" CHECK (("category" = ANY (ARRAY['DISCOUNT'::"text", 'CONSTRAINT'::"text", 'FEE'::"text", 'BUNDLE_ADJUSTMENT'::"text"]))),
    CONSTRAINT "pricing_rules_priority_check" CHECK ((("priority" >= 0) AND ("priority" <= 1000))),
...
COMMENT ON TABLE "public"."pricing_rules" IS 'New pricing rules engine 
  - structured rule definitions with category-based 
  execution';



COMMENT ON COLUMN "public"."pricing_rules"."category" IS 'Rule category: 
  DISCOUNT, CONSTRAINT, FEE, BUNDLE_ADJUSTMENT';



COMMENT ON COLUMN "public"."pricing_rules"."conditions" IS 'Array of rule conditions (can be empty for universal rules)';



COMMENT ON COLUMN "public"."pricing_rules"."actions" IS 'Array of rule 
  actions using ActionType enum';
...
CREATE OR REPLACE VIEW "public"."pricing_rules_field_audit" AS
 SELECT "id",
    "name",
    "category",
        CASE
            WHEN (("conditions")::"text" ~~ '%state.%'::"text") THEN 'Has state references'::"text"
            WHEN (("conditions")::"text" ~~ '%request.group%'::"text") THEN 'Has request.group'::"text"
            WHEN (("conditions")::"text" ~~ '%request.region%'::"text") THEN 'Has request.region'::"text"
            ELSE 'Clean'::"text"
        END AS "field_status",
    "conditions"
...
ALTER VIEW "public"."pricing_rules_field_audit" OWNER TO "postgres";
...
CREATE INDEX "idx_pricing_rules_category" ON "public"."pricing_rules" USING "btree" ("category");
...
CREATE OR REPLACE TRIGGER "update_pricing_rules_updated_at" BEFORE UPDATE ON "public"."pricing_rules" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
GRANT ALL ON TABLE "public"."pricing_rules_field_audit" TO "anon";
GRANT ALL ON TABLE "public"."pricing_rules_field_audit" TO "authenticated";
GRANT ALL ON TABLE "public"."pricing_rules_field_audit" TO "service_role";
