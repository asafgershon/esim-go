...
CREATE TABLE catalog_bundles (
  ...
  bundle_id VARCHAR NOT NULL UNIQUE,              -- Bundle ID from eSIM Go API
  name VARCHAR NOT NULL,                          -- Bundle name from eSIM Go API
  bundle_group VARCHAR,                           -- Standard Fixed, Standard - Unlimited, etc.
  description TEXT,                               -- Bundle description
  duration INTEGER,                               -- Duration in days
  data_amount BIGINT,                             -- Data amount in MB (-1 for unlimited)
...
CREATE TABLE catalog_metadata (
  ...
  bundle_groups TEXT[],                           -- Available bundle groups as array
...
-- Create permissive policies for now (restrict later as needed)
